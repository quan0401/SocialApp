...
export class AuthService {
  ...
  public async findAndUpdatePasswordByEmail(email: string, newPassword: string) {
    const updated = await AuthModel.findOneAndUpdate({ email: email }, { password: newPassword }, { new: true, upsert: false });
    return updated;
  }
